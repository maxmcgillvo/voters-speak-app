// Executive Branch Focused Contact Data
// White House Leadership & Cabinet Secretaries Only
// Updated: October 12, 2025 - Department of Defense → Department of War

const executiveData = [
    // ===========================
    // WHITE HOUSE LEADERSHIP
    // ===========================
    {
        name: "Donald J. Trump",
        title: "President",
        party: "Republican",
        email: "comments@whitehouse.gov",
        phone: "(202) 456-1111",
        office: "The White House",
        website: "https://www.whitehouse.gov",
        address: "1600 Pennsylvania Ave NW, Washington, DC 20500",
        description: "45th and 47th President of the United States",
        category: "White House Leadership"
    },
    {
        name: "JD Vance",
        title: "Vice President",
        party: "Republican",
        email: "comments@whitehouse.gov",
        phone: "(202) 456-1111",
        office: "The White House",
        website: "https://www.whitehouse.gov",
        address: "1600 Pennsylvania Ave NW, Washington, DC 20500",
        description: "50th Vice President of the United States",
        category: "White House Leadership"
    },
    {
        name: "Susie Wiles",
        title: "White House Chief of Staff",
        party: "Republican",
        email: "chief.staff@who.eop.gov",
        phone: "(202) 456-1414",
        office: "The White House",
        website: "https://www.whitehouse.gov",
        address: "1600 Pennsylvania Ave NW, Washington, DC 20500",
        description: "First female White House Chief of Staff",
        category: "White House Leadership"
    },
    {
        name: "Karoline Leavitt",
        title: "White House Press Secretary",
        party: "Republican",
        email: "press@who.eop.gov",
        phone: "(202) 456-1414",
        office: "The White House",
        website: "https://www.whitehouse.gov",
        address: "1600 Pennsylvania Ave NW, Washington, DC 20500",
        description: "Primary spokesperson for the President and White House",
        category: "White House Leadership"
    },
    {
        name: "Michael Waltz",
        title: "National Security Advisor",
        party: "Republican",
        email: "nsc@who.eop.gov",
        phone: "(202) 456-1111",
        office: "The White House",
        website: "https://www.whitehouse.gov",
        address: "1600 Pennsylvania Ave NW, Washington, DC 20500",
        description: "Principal advisor to the President on national security",
        category: "White House Leadership"
    },
    {
        name: "Dan Scavino",
        title: "White House Deputy Chief of Staff",
        party: "Republican",
        email: "deputy.chief@who.eop.gov",
        phone: "(202) 456-1414",
        office: "The White House",
        website: "https://www.whitehouse.gov",
        address: "1600 Pennsylvania Ave NW, Washington, DC 20500",
        description: "Deputy Chief of Staff for strategic communications",
        category: "White House Leadership"
    },
    {
        name: "Stephen Miller",
        title: "White House Deputy Chief of Staff for Policy",
        party: "Republican",
        email: "policy.advisor@who.eop.gov",
        phone: "(202) 456-1414",
        office: "The White House",
        website: "https://www.whitehouse.gov",
        address: "1600 Pennsylvania Ave NW, Washington, DC 20500",
        description: "Senior policy advisor on immigration and homeland security",
        category: "White House Leadership"
    },

    // ===========================
    // CABINET SECRETARIES
    // ===========================
    {
        name: "Marco Rubio",
        title: "Secretary of State",
        party: "Republican",
        email: "secretary@state.gov",
        phone: "(202) 647-4000",
        office: "Department of State",
        website: "https://www.state.gov",
        address: "2201 C Street NW, Washington, DC 20520",
        description: "Chief diplomat and foreign policy advisor to the President",
        category: "Cabinet Secretaries"
    },
    {
        name: "Scott Bessent",
        title: "Secretary of the Treasury",
        party: "Republican",
        email: "secretary@treasury.gov",
        phone: "(202) 622-2000",
        office: "Department of Treasury",
        website: "https://www.treasury.gov",
        address: "1500 Pennsylvania Avenue NW, Washington, DC 20220",
        description: "Manages federal finances, economic policy, and financial regulations",
        category: "Cabinet Secretaries"
    },
    {
        name: "Pete Hegseth",
        title: "Secretary of War",
        party: "Republican",
        email: "secretary@defense.gov",
        phone: "(703) 571-3343",
        office: "Department of War",
        website: "https://www.defense.gov",
        address: "1000 Defense Pentagon, Washington, DC 20301",
        description: "Oversees all branches of the U.S. military and national defense",
        category: "Cabinet Secretaries"
    },
    {
        name: "Pam Bondi",
        title: "Attorney General",
        party: "Republican",
        email: "contact@usdoj.gov",
        phone: "(202) 514-2000",
        office: "Department of Justice",
        website: "https://www.justice.gov",
        address: "950 Pennsylvania Ave NW, Washington, DC 20530",
        description: "Chief law enforcement officer and head of the Department of Justice",
        category: "Cabinet Secretaries"
    },
    {
        name: "Doug Burgum",
        title: "Secretary of the Interior",
        party: "Republican",
        email: "contact@ios.doi.gov",
        phone: "(202) 208-3100",
        office: "Department of the Interior",
        website: "https://www.doi.gov",
        address: "1849 C St NW, Washington, DC 20240",
        description: "Manages federal lands, natural resources, and environmental policy",
        category: "Cabinet Secretaries"
    },
    {
        name: "Brooke Rollins",
        title: "Secretary of Agriculture",
        party: "Republican",
        email: "contact@usda.gov",
        phone: "(202) 720-2791",
        office: "Department of Agriculture",
        website: "https://www.usda.gov",
        address: "1400 Independence Ave SW, Washington, DC 20250",
        description: "Oversees farming, food safety, and agricultural policy",
        category: "Cabinet Secretaries"
    },
    {
        name: "Howard Lutnick",
        title: "Secretary of Commerce",
        party: "Republican",
        email: "contact@doc.gov",
        phone: "(202) 482-2000",
        office: "Department of Commerce",
        website: "https://www.commerce.gov",
        address: "1401 Constitution Ave NW, Washington, DC 20230",
        description: "Promotes economic growth and regulates business and trade",
        category: "Cabinet Secretaries"
    },
    {
        name: "Lori Chavez-DeRemer",
        title: "Secretary of Labor",
        party: "Republican",
        email: "contact@dol.gov",
        phone: "1-866-4-USA-DOL",
        office: "Department of Labor",
        website: "https://www.dol.gov",
        address: "200 Constitution Ave NW, Washington, DC 20210",
        description: "Oversees labor laws, workplace safety, and workforce development",
        category: "Cabinet Secretaries"
    },
    {
        name: "Robert F. Kennedy Jr.",
        title: "Secretary of Health and Human Services",
        party: "Republican",
        email: "contact@hhs.gov",
        phone: "877-696-6775",
        office: "Department of Health and Human Services",
        website: "https://www.hhs.gov",
        address: "200 Independence Ave SW, Washington, DC 20201",
        description: "Oversees public health, Medicare, Medicaid, and medical research",
        category: "Cabinet Secretaries"
    },
    {
        name: "Scott Turner",
        title: "Secretary of Housing and Urban Development",
        party: "Republican",
        email: "contact@hud.gov",
        phone: "202-708-1112",
        office: "Department of Housing and Urban Development",
        website: "https://www.hud.gov",
        address: "451 7th St SW, Washington, DC 20410",
        description: "Manages housing programs and urban development initiatives",
        category: "Cabinet Secretaries"
    },
    {
        name: "Sean Duffy",
        title: "Secretary of Transportation",
        party: "Republican",
        email: "contact@dot.gov",
        phone: "(202) 366-4000",
        office: "Department of Transportation",
        website: "https://www.transportation.gov",
        address: "1200 New Jersey Ave SE, Washington, DC 20590",
        description: "Oversees transportation infrastructure, safety, and regulations",
        category: "Cabinet Secretaries"
    },
    {
        name: "Chris Wright",
        title: "Secretary of Energy",
        party: "Republican",
        email: "contact@energy.gov",
        phone: "(202) 586-5000",
        office: "Department of Energy",
        website: "https://www.energy.gov",
        address: "1000 Independence Ave SW, Washington, DC 20585",
        description: "Manages energy policy, nuclear security, and energy research",
        category: "Cabinet Secretaries"
    },
    {
        name: "Linda McMahon",
        title: "Secretary of Education",
        party: "Republican",
        email: "contact@ed.gov",
        phone: "1-800-USA-LEARN",
        office: "Department of Education",
        website: "https://www.ed.gov",
        address: "400 Maryland Ave SW, Washington, DC 20202",
        description: "Oversees federal education policy and student financial aid",
        category: "Cabinet Secretaries"
    },
    {
        name: "Kristi Noem",
        title: "Secretary of Homeland Security",
        party: "Republican",
        email: "contact@dhs.gov",
        phone: "(202) 282-8000",
        office: "Department of Homeland Security",
        website: "https://www.dhs.gov",
        address: "3801 Nebraska Ave NW, Washington, DC 20528",
        description: "Oversees national security, immigration, and disaster response",
        category: "Cabinet Secretaries"
    },
    {
        name: "Doug Collins",
        title: "Secretary of Veterans Affairs",
        party: "Republican",
        email: "contact@va.gov",
        phone: "1-800-827-1000",
        office: "Department of Veterans Affairs",
        website: "https://www.va.gov",
        address: "810 Vermont Ave NW, Washington, DC 20420",
        description: "Oversees veterans' benefits, healthcare, and services",
        category: "Cabinet Secretaries"
    }
];

// Export for use in main application
if (typeof module !== 'undefined' && module.exports) {
    module.exports = executiveDataFocused;
}